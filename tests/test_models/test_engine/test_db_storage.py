...
class TestFileStorage(unittest.TestCase):
    ...
    @unittest.skipIf(models.storage_t != 'db', "not testing db storage")
    def test_save(self):
        """Test that save properly saves objects to file.js"""


class test_db_storage(unittest.TestCase):
    """
    Tests the db storage class
    """
    def test_get(self):
        '''
            Test if get retirves stored objecs
        '''
    ...
    def test_count(self):
        '''
            Tests the count functionality
            if returns correct object nos
        '''
        storage.reload()
        old_count = storage.count("State")
        new_state1 = State(name="Ohio")
        storage.new(new_state1)
        new_state2 = State(name="Oregon")
